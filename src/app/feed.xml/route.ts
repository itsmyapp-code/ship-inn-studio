  ...
  allItems.forEach((item) => {
    ...
    const description =
      item.type === 'event'
        ...
        : ((item.entry as any).content && (item.entry as any).content[0] && (item.entry as any).content[0].children && (item.entry as any).content[0].children.map((c: any) => c.text).join('')) || 'Latest news from The Ship Inn.'
